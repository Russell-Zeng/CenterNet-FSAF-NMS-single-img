...
class DLASeg(nn.Module):
    def __init__(self, base_name, heads, pretrained, down_ratio, final_kernel, last_level, head_conv, out_channel=0):
        ...
        # ========================输出的3个分支不共享参数, 对3个特征图分别定义了一套预测分支网络
        self.heads = heads
            ...
            for head in self.heads:
                ...
                if head_conv > 0:
                    ...
                    if 'hm' in head:
                        ...
                        fill_fc_weights(fc)
                self.__setattr__(head+str(map_idx), fc)
    ...
    def forward(self, x):
        ...
        for map_idx, out0 in enumerate(y):   #　 y[0]->y[1]->y[2] ,虽然特征图尺寸一致，但是y[0]中特征图高分辨率信息多，应该用来回归小物体；y[2]用来回归大物体
            dla_fpn_z = {}
            # print('self.heads:', self.heads)
            for head in self.heads:
                # print(head, 'before:', out0.size())
                # print(head, 'struture:', self.__getattr__(head+str(map_idx)))
                dla_fpn_z[head] = self.__getattr__(head+str(map_idx))(out0)
                # print(head, 'after:', dla_fpn_z[head].size())

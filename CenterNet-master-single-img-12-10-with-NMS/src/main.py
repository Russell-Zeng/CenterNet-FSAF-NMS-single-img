...
import _init_paths
# from src.tools.init_paths import init_paths
import os
...
def main(opt):
  torch.manual_seed(opt.seed)   # opt.seed: default=317  ；加上torch.manual_seed这个函数调用的话，打印出来的随机数每次都一样。
  torch.backends.cudnn.benchmark = not opt.not_cuda_benchmark and not opt.test
  Dataset = get_dataset(opt.dataset, opt.task) # opt.dataset = coco, opt.task = ctdet (| ddd | multi_pose | exdet)
  ...
  for epoch in range(start_epoch + 1, opt.num_epochs + 1):
    ...
    try:
      log_dict_train, _ = trainer.train(epoch, train_loader) # !!!!!!!!  train = self.run_epoch('train', epoch, data_loader)

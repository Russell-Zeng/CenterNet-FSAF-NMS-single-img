...
def _nms(heat, kernel=3):   # 这个NMS的实现对吗？？？？？？？？？？？？
    pad = (kernel - 1) // 2  # 1

    hmax = nn.functional.max_pool2d(heat, (kernel, kernel), stride=1, padding=pad)   # 这个最大池化会保持特征图尺寸不变
    keep = (hmax == heat).float()  # 这个相当于掩码
    return heat * keep
...
def ctdet_decode(heat, wh, reg=None, cat_spec_wh=False, K=100):
    # ctdet_decode(output['hm'], output['wh'], reg=reg, cat_spec_wh=self.opt.cat_spec_wh, K=self.opt.K)

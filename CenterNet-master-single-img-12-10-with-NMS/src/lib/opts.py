...
class opts(object):
  def __init__(self):
    self.parser = argparse.ArgumentParser()  # 实例化一个参数解析器对象
    ...
    self.parser.add_argument('--exp_id', default='coco_dla_img_fpn_12-30') #　exp_id只是一个名字，为了区分每次实验，没有特别的作用
    self.parser.add_argument('--test', action='store_true')
    # self.parser.add_argument('--test', action='store_true', default=True)
    ...
    self.parser.add_argument('--demo', default='/home/zy/zy/2new_network/CenterNet-master/data/coco/images/val2017',
                             help='path to image/ image folders/ video. '
                                  'or "webcam"')   # 这个参数是用来做演示的
    # default='/home/zy/zy/2new_network/CenterNet-master/exp/ctdet/coco_dla3/model_best.pth'
    self.parser.add_argument('--load_model', default='',
                             help='path to pretrained model')  #  测试的时候需要！！！！！！！！！！！！！！！！！！！！
    # self.parser.add_argument('--load_model', default='',
    ...
    self.parser.add_argument('--resume', action='store_true',
                             help='resume an experiment. '
                                  ...
                                  'in the exp dir if load_model is empty.')    # 这个参数是用来恢复上次的训练
    ...
    # log
    self.parser.add_argument('--print_iter', type=int, default=1,   # 设为0意思是不再终端显示训练progress_bar。设为1 意思每个iter都要显示
                             help='disable progress bar and print to screen.')
    self.parser.add_argument('--hide_data_time', action='store_true',
                             help='not display time during training.')
    self.parser.add_argument('--save_all', action='store_true',
                             help='save model to disk every 5 epochs.')  # ???????????every 5 epochs????
    ...
    self.parser.add_argument('--arch', default='dla_34',
                             help='model architecture. Currently tested'
                                  ...
                                  'dlav0_34 | dla_34 | hourglass | new-network | dla-matrixnet')
    ...
    # train
    self.parser.add_argument('--lr', type=float, default=0.625e-4,    #
                             help='learning rate for batch size 32.')
    self.parser.add_argument('--lr_step', type=str, default='50,60',
                             help='drop learning rate by 10.')
    self.parser.add_argument('--num_epochs', type=int, default=70,  # 原本是140轮，由于此次数据增多了，增加到180轮
                             help='total training epochs.')
    self.parser.add_argument('--batch_size', type=int, default=4,
                             help='batch size')             # 原来的batch_size = 32
    self.parser.add_argument('--master_batch_size', type=int, default=-1,
                             help='batch size on the master gpu.')
    self.parser.add_argument('--num_iters', type=int, default=-1,
                             help='default: #samples / batch_size.')
    self.parser.add_argument('--val_intervals', type=int, default=10,
                             help='number of epochs to run validation.')  # validation不知道这次有没有用？？？？？
    self.parser.add_argument('--trainval', action='store_true',
                             help='include validation in training and '
                                  'test on test set')   # ????可能会出错　，　因为我的ｒｉｎｇ数据集里的ｔｒａｉｎ数据集里就已经包括了ｖａｌ
    ...
    self.parser.add_argument('--not_rand_crop', action='store_true',
                             help='not use the random crop data augmentation'
                                  'from CornerNet.')   # 逻辑是：如果不使用随机裁剪，就会使用尺度变换;而尺度变换对项目提升更高
  ...
  def parse(self, args=''):
    ...
    opt.gpus_str = opt.gpus  # prase argument about gpu
    opt.gpus = [int(gpu) for gpu in opt.gpus.split(',')]
    opt.gpus = [i for i in range(len(opt.gpus))] if opt.gpus[0] >=0 else [-1]
    opt.lr_step = [int(i) for i in opt.lr_step.split(',')]   # prase argument about lr_step
    opt.test_scales = [float(i) for i in opt.test_scales.split(',')]

    opt.fix_res = not opt.keep_res  # if opt.keep_res: keep the original resolution during validation
    print('Fix size testing.' if opt.fix_res else 'Keep resolution testing.')
    opt.reg_offset = not opt.not_reg_offset    # if opt.not_reg_offset : not regress local offset
    opt.reg_bbox = not opt.not_reg_bbox    # if opt.not_reg_bbox: not regression bounding box size
    opt.hm_hp = not opt.not_hm_hp   # if opt.not_hm_hp: not estimate human joint heatmap
    opt.reg_hp_offset = (not opt.not_reg_hp_offset) and opt.hm_hp

    # opt.head_conv: conv layer channels for output head
    if opt.head_conv == -1: # init default head_conv
      opt.head_conv = 256 if 'dla' in opt.arch else 64
    opt.pad = 127 if 'hourglass' in opt.arch else 31   #  opt.pad  for what???
    opt.num_stacks = 2 if opt.arch == 'hourglass' else 1

    if opt.trainval:   # ?????如果trainval = True ,那么就没有中间的ｖａｌ步骤？？？？？？
      opt.val_intervals = 1000000

    if opt.debug > 0:  # 若opt.debug>0,则开启训练过程中的可视化调试部分
      opt.num_workers = 0
      opt.batch_size = 1
      opt.gpus = [opt.gpus[0]]
    ...
    opt.chunk_sizes = [opt.master_batch_size]   #  chunk_sizes?????
    ...
    # opt.root_dir = os.path.join(os.path.dirname(__file__), '..', '..')  # 这行代码也是为了获取上上级目录，但是好像没起作用
    opt.root_dir = os.path.abspath(os.path.join(os.getcwd(), ".."))    # 获取上级目录
    ...
    if opt.resume and opt.load_model == '':    #   用来恢复上次训练
      model_path = opt.save_dir[:-4] if opt.save_dir.endswith('TEST') \
                  else opt.save_dir
      opt.load_model = os.path.join(model_path, 'model_last.pth')
    return opt

  def update_dataset_info_and_set_heads(self, opt, dataset): # 为什么dataset这个对象可以访问COCO, CTDetDataset这两个类中的属性？？？？？？？？？？？？？？？？
    input_h, input_w = dataset.default_resolution  # [512,512]
    # print("333333", dataset.what)
  ...
  def init(self, args=''):
    ...
    class Struct:
      ...
      def __setattr__(self, key, value):  # add by zengyuan

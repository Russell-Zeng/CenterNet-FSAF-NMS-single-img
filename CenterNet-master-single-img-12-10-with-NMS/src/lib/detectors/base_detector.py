...
class BaseDetector(object):
  ...
  def run(self, image_or_path_or_tensor, meta=None):
    ...
    elif type(image_or_path_or_tensor) == type (''):  # 经过demo.py的处理：image_or_path_or_tensor是单张图片的路径
    ...
    for scale in self.scales:  # self.scales = 1 # 这个参数等于2或者0.5也可以，但是等于1检测效果最好
      ...
      print(scale, images.size())
      output, dets, forward_time = self.process(images, return_time=True)  # 这一步是底层的检测，是针对nparray数组的

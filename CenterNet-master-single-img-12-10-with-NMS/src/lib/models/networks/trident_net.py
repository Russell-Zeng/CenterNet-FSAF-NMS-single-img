...
class Trident_BasicBlock(nn.Module):
    def __init__(self, inplanes, planes, stride=1, downsample=None, dilation=1, padding=[1, 2, 3], dilate=[1, 2, 3]):
        super(Trident_BasicBlock, self).__init__()
        self.stride = stride
        self.padding = padding
        self.dilate = dilate
        self.downsample = downsample

        self.share_weight4conv1 = nn.Parameter(torch.randn(planes, inplanes, 3, 3))
        ...
        self.cat_conv = nn.Conv2d(planes*3, planes, kernel_size=1,
                               stride=1, padding=0,
                               bias=False, dilation=1)
        self.ca = ChannelAttention(planes*3)
        self.sa = SpatialAttention()

    def forward_for_small(self, x, residual=None):
        if residual is None:
            residual = x
        out = nn.functional.conv2d(x, self.share_weight4conv1, bias=None, stride=self.stride, padding=self.padding[0], dilation=self.dilate[0])
        out = self.bn11(out)
        out = self.relu1(out)

        out = nn.functional.conv2d(out, self.share_weight4conv2, bias=None, padding=self.padding[0])
        out = self.bn12(out)
        ...
        return out
    def forward_for_middle(self, x, residual=None):
        if residual is None:
            residual = x
        out = nn.functional.conv2d(x, self.share_weight4conv1, bias=None, stride=self.stride, padding=self.padding[1],dilation=self.dilate[1])
        out = self.bn21(out)
        out = self.relu2(out)
        ...
        return out
    def forward_for_big(self, x, residual=None):
        if residual is None:
            residual = x
        out = nn.functional.conv2d(x, self.share_weight4conv1, bias=None, stride=self.stride, padding=self.padding[2], dilation=self.dilate[2])
        out = self.bn31(out)
        out = self.relu3(out)
    ...
    def forward(self, x, residual=None):
        ...
        #     out = torch.cat([x1, x2, x3], dim=1)
        #     out = self.cat_conv(out)
        x1 = self.forward_for_small(x, residual)
        # print(222, x.size())
        base_feat.append(x1)
        x2 = self.forward_for_middle(x, residual)
        # print(333, x.size())
        base_feat.append(x2)
        x3 = self.forward_for_big(x, residual)
        # print(444, x.size())
        base_feat.append(x3)
        out = torch.cat([x1, x2, x3], dim=1)  # 用这个out作为注意力通道的输入
        out = self.ca(out) * out  # 引入注意力机制
        out = self.sa(out) * out

        out = self.ReLU(out)
        # print(555, out.size())
        out = self.cat_conv(out) # 将3个分支的特征图融合
        out = self.BN(out)
...
class Trident_BasicBlock2(nn.Module):
    def __init__(self, inplanes, planes, stride=1, dilation=1):
        super(BasicBlock, self).__init__()
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=3,
                               stride=stride, padding=dilation,
...
class Tree(nn.Module):
    # Tree(1, block, 32, 64, 2, level_root=False,root_residual=residual_root)
    # Tree(2, block, 64, 128, 2, level_root=False,root_residual=residual_root)
    # Tree(2, block, 128, 256, 2, level_root=False,root_residual=residual_root)
    # Tree(1, block, 256, 512, 2, level_root=False,root_residual=residual_root)
...
def get_pose_net(num_layers, heads, head_conv=256, down_ratio=4):
    model = DLASeg('dla{}'.format(num_layers), heads,
                   pretrained=False,  # 本来pretrained = True，为了做实验才设为False

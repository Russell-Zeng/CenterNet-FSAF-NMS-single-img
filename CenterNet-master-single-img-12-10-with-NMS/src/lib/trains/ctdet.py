  ...
  def forward(self, all_outputs, batch):
    ...
    for i in range(batch_size):
      ...
      if real_obj_num > 0:  #!!!!!!!!!!有些图像因为随机裁剪所以其中没有一个物体
        ...
        output_off_loss = []
        output_loss_sum = 0
        output_hm_loss_sum = 0
        output_wh_loss_sum = 0
        output_off_loss_sum = 0
        for key, outputs in enumerate(all_outputs):
          ...
          output_loss_sum += loss  #  add by zy 12-19 !!!!!!!!!!!!!!!!!!!! 将所有物体在所有fpn层上回归
          output_hm_loss_sum += hm_loss
          output_wh_loss_sum += wh_loss
          output_off_loss_sum += off_loss
        ...
        img_loss = output_loss_sum  # img_loss是一张图像上所有obj的最小loss总和
        img_hm_loss = output_hm_loss_sum
        img_wh_loss = output_wh_loss_sum
        img_off_loss = output_off_loss_sum
